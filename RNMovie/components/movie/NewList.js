...
export default class MovieList extends React.Component {
    ...
    renderItem = (item, index) => {
        ...
        return <TouchableHighlight underlayColor='#fff' onPress={() => this.getMovieDetail(item.id)}>
            <View style={{ flexDirection: 'row', justifyContent: "flex-start", height: 160 }}>
                <View>
                    {choose(index)}
                    <Image source={{ uri: item.images.small }} style={{ width: 90, height: 110, marginLeft: 7 }}></Image>
                </View>
                <View style={{ paddingLeft: 10,position: 'relative' }}>
                    <Text style={{ paddingTop: 32, fontSize: 18 }}>{item.title}</Text>
                    <View style={{ paddingTop: 10, width: 50, flexDirection: 'row' }}>
                        <StarRating
                            disabled={false}
                            maxStars={5}
                            starSize={9}
                        ...
                        />
                        <Text style={{ fontSize: 12, marginLeft: 6, position: 'absolute', left: 45, top: 7 }}>{item.rating.average}</Text>
    ...
    getMovieDatas = () => {
        const url1 = `http://api.douban.com/v2/movie/new_movies?apikey=0df993c66c0c636e29ecbb5344252a4a&start=${this.state.start}&count=${this.state.count}`
            ...
            .then(data => {
                ...
                this.setState(state => {
                    return {
                        ...
                        photo: data.subjects[0].images.small,
                        title: data.title,
                        isloading: false,
